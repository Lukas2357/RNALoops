...
from rnaloops.data_explorer.explore_fcts import get_prepared_df
...
class PdbStructure:
    ...
    def get_bonds_df(self):
        ...
        for base in base_order.index.values:
            if base not in bonds_df.index.get_level_values(level=0):
                bonds_df.loc[(base, '-', '-', '-', '-', '-'), :] = None

        try:
            bonds_df = bonds_df.reindex(base_order.index.values, level=0)
        except ValueError:
    ...
    def get_base_order(self):
        ...
        base_order_df = pd.DataFrame(index=base_order)
...
def get_full_structures(idx_min=0, idx_max=0, df=None):
    ...
    with open(f'structures/pdb_full_structures_{idx_min}-{idx_max}.pkl',
              'wb') as f:
...
def load_full_structures(idx_min=0, idx_max=0):
    with open(f'structures/pdb_full_structures_{idx_min}-{idx_max}.pkl',
              'rb') as f:
