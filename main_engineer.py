...
n_clusters = [25, 50, 75, 100, 150, 200]  # Number of initial clusters
alg = 'agglomerative'  # The algorithm for initial clustering (string)
chunk_size = 2  # Size of chunks to evaluate in one Pool at a time
resets = 10  # Number of resets (to release memory)

user_params = dict(
    save=True,  # Whether to save cluster id plots (bool)
    max_devs=[2, 4, 6, 8],  # Max angle deviation for cluster (Iterable)
    min_density=0,  # Minimum density to save cluster (Integer)
    min_n=10,  # Minimum number of sequences needed for cluster (Integer)
)
...
agg_all = load_agg_df()[[f'planar_{i}_mean' for i in range(1, 15)] +
                        [f'planar_{i}_median' for i in range(1, 15)]]

for way in range(3, 9):

    agg_df = load_agg_df(way=way)
